		...
		<div class="product" v-for="item in products" :key="item.id">				
			<div class="max-w-sm rounded overflow-hidden shadow-lg h-full">
				...
				<div class="flex flex-col justify-between">
					<div class="font-bold text-lg text-center mb-2 p-3 h-20 truncate-2">{{ item.title }}</div>
					<div class="flex items-center justify-end mx-4 mb-4">
						<p class="text-2xl text-blue-800 font-bold mr-3">RM{{ item.price }}</p>
						<button class="bg-blue-500 focus:outline-none hover:bg-blue-700 text-white font-bold rounded-full h-12 w-12 p-3">
